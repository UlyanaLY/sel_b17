...
public class MyFifthTest extends TestBase {
    ...
    @Test
    public void areGeoZonesSorted() {
        driver.navigate().to("http://localhost/litecart/admin/?app=geo_zones&doc=geo_zones");
        List<WebElement> countries = driver.findElements(xpath("//table[@class='dataTable']//tr//td[3]"));
        int countriesSize = countries.size();
        ArrayList<String> countriesNameSorted = new ArrayList<>();
        ArrayList<String> countriesNameAnSorted = new ArrayList<>();
        ...
        System.out.println(countriesNameSorted);
        System.out.println(countriesNameAnSorted);
        Collections.sort(countriesNameSorted);
        Assert.assertEquals(countriesNameSorted, countriesNameAnSorted);

        //Проверить, что зоны отсортированы
        for (int j = 1; j <= countries.size(); j++) {
            if (!driver.findElement(xpath("//tr[@class='row'][" + j + "]//td[4]")).getText().equals("0")) {
                driver.findElement(xpath("//tr[@class='row'][" + j + "]//td[5]")).click();
                List<WebElement> zones = driver.findElements(xpath("//table[@class='dataTable']//tr//td[3]//select/option[@selected='selected']"));
                ...
                driver.findElement(xpath(("//ul[@id='box-apps-menu']//li[6]"))).click();
            }
        }
    }
}
